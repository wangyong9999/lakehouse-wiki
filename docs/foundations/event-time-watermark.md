...
- **CDC 入湖的"新鲜度"** = Watermark 推进速度
  ...
  - Paimon / Iceberg 的 commit 触发由 checkpoint + watermark 共同决定
- **Paimon Changelog Producer 的 `lookup` 模式** 需要强 Watermark 对齐
- **Iceberg 流式读（incremental_changes）** 依赖上游 Watermark 确保不漏 snapshot
