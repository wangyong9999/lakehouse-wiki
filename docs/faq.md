...
**spec 已于 2025-06 ratified · 引擎侧 rolling out**（Spark 4.0+ / Trino 450+ 已部分支持）。2026-Q2 生产扩散中 · 可按需采用：

- **早期采用**：高频删除（Deletion Vector）· 合规严（Row Lineage）· 多表原子事务 · GIS（Geo 类型）
- **v2 可继续**：无明确痛点 · 引擎版本未到位 · 小规模
- **迁移时**：客户端全栈升级 + 灰度 + Catalog 支持 v3 协议

→ [Iceberg v3](lakehouse/iceberg-v3.md)
...
**一定要先建评估集再调**，否则盲飞。
→ [RAG on Lake](scenarios/rag-on-lake.md) · [RAG §4 高级范式](ai-workloads/rag.md)
