...
> **Equality delete 只应用于 sequence-number 小于或等于该 delete 的 data file**

这样 commit 12 的新 insert（sequence-number=12）不会被 commit 11 的 equality delete（sequence-number=11）误删。
...
```python
def read_data_file(data_file, ref_delete_files, ref_dv_blobs, snap_seq):
    # 1. 收集所有应用于 data_file 的 delete 载体
    deletes_to_apply = [
        d for d in ref_delete_files + ref_dv_blobs
        if d.applies_to(data_file)
           and d.sequence_number <= snap_seq  # sequence number 规则
    ]
