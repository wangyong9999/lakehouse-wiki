...
applies_to: Iceberg v3, Paimon 1.0+, Lance v2, Vortex, GeoParquet
...
**Variant 是 Spark 3.4+ 引入的新类型**（2024），**Iceberg v3 吸收**（2025-06 ratified）：
...
3. **GraphAr**（Alibaba 开源，2023+）：图数据的列式存储格式，和 Parquet / Iceberg 对齐
...
**Vortex**（Spiral Labs, 2024 开源）：新一代列存格式，承诺在**三种负载上都优于 Parquet**。如成立会是下一代湖表基底。

## 前沿研究 · 2026 可关注

- **Lance v2** (CIDR 2024) —— ML workload 优先的列存
- **Vortex**（Spiral Labs, 2024）—— "Parquet 继承者"候选
...
- [Lance Format](../foundations/lance-format.md) —— 向量原生
- [Puffin](puffin.md) —— 索引侧车（向量未来承载）
- [向量数据库](../retrieval/vector-database.md) —— Milvus / LanceDB / Qdrant
- [对象存储](../foundations/object-storage.md) —— 物理底座

## 延伸阅读

- **Lance v2 · CIDR 2024** —— ML 工作负载列式格式
- **Vortex: 2.0 Columnar Format** (Spiral Labs, 2024+)
