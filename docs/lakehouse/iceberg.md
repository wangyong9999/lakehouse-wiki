...
v3 是 Iceberg 近几年最大一波能力补强。**spec 本身于 2025-06 正式 ratified**（Apache Iceberg project vote 通过）；引擎侧 rolling out：

- Iceberg **1.8.0**（2025-02）引入 Binary Deletion Vectors + Variant
- **1.9.0**（2025-04）扩展 Row Lineage + Default Values + Multi-arg Transforms
- **1.10.0**（2025-09）进一步成熟，DV "ready for prime time"
...
**选型指引**：新项目可直接上 v3（Spark 4.0 栈尤其推荐）；混合引擎环境先确认**消费侧 reader 都已升级**——老 reader 读 v3 表会失败。
...
### 机制 8 · Materialized View · 湖上增量物化（2024+ spec proposal）

Iceberg MV spec 2024 进入 community proposal / 部分引擎预览实现（Trino 1.8+ / Spark 迭代中）。核心思路：

- **MV 底层是一张真实的 Iceberg 表** + 指向源表的**刷新状态记录**
- 定期（或触发式）**增量刷新**：对比源表 snapshot 差量，只应用变化
- **Stale detection**：查询时可选"接受 stale MV" / "穿透到源表"两种语义

**相比 Paimon / Delta MV**：
- Paimon 有 "table-store aggregation" 做类 MV；Delta 有 Materialized View（Databricks 专属）
- Iceberg MV 走**协议标准化**路线——多引擎能共享同一份 MV 定义与数据
...
**关键观察**：**Iceberg 从"表格式"进化为"云厂商的统一 Catalog 协议"**。选 Iceberg 越来越像选 "HTTP"——它不是最好，而是所有人都支持。
