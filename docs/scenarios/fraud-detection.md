...
### Uber · 实时欺诈检测（Michelangelo + 图分析）

**为什么值得学**：Uber 业务天生 ML-first · 欺诈检测是其**实时 ML 决策**代表场景（支付欺诈 · 虚假司机 · 刷单）。**全栈视角见 [cases/uber](../cases/uber.md)**。

**欺诈场景独特做法**：

1. **四层拦截的"Uber 版"**（和本章四层架构高度对齐）：
   - 黑白名单 → 规则引擎 → **Michelangelo ML** → **图分析**（团伙欺诈）
   - 核心卖点：**实时 · sub-second 决策**（支付 / 匹配时点）

2. **Michelangelo 在欺诈场景的作用**：
   - **训推一致性** —— 欺诈场景最敏感（特征漂移 → 漏抓 / 误杀）
   - **Palette / Genoa Feature Store**：实时用户行为 + 设备画像 + 地理位置 · ms 级
   - 多模型并行（支付 / 账号 / 司机 各自）

3. **图分析拦截团伙欺诈**：
   - User - Device - Payment 三元图
   - 团伙欺诈（同设备开多账号 / 相互刷单）**单看用户看不出** · 图结构必需
   - Uber 用**实时图查询**做决策 · 工业前沿

**规模** `[量级参考]`：在线 ML 决策**数十亿 / 日** · 欺诈场景 p99 **sub-second**。

**踩坑**（来自 [cases/uber §9](../cases/uber.md)）：Michelangelo 早期过度复杂化 · 实时图存储自建成本高。

**对照与复用**：
- ✅ 四层完全对齐
- ⚠️ Uber 量级（数十亿决策）· 中型团队**只要做到前三层**（规则 + ML）· GNN 按规模决定
- ⚠️ 实时图查询对多数团队过度 · 批图 + 实时规则组合更经济

### LinkedIn · 内容安全 / 账号欺诈（推断）

!!! warning "以下为合理推断 · LinkedIn 内容安全具体方案公开较少"
    - **数据底座**：Kafka 全家桶 · 事件流驱动
    - **特征在线 store**：**Venice**（百万 QPS · 推测用于欺诈 Feature Store）
    - **ML 平台**：Feathr（Feature Store · 2024 捐 Apache）
    - **核心能力**：**结构化 + 行为事件**混合特征 · LinkedIn 擅长的 LTR 思想可能扩展到欺诈打分
    - 以 LinkedIn Engineering Blog 最新发布为准

### 共同规律（两家事实观察）

- 欺诈检测**强依赖实时 Feature Store**（特征延迟 > 秒级 → 欺诈已成）
- **多层拦截架构**是共识（不是"一个模型搞定"）
- **标签延迟**是欺诈场景特有问题（详见 §标签延迟）

### 对中型团队的启示（事实观察）

- **不要照搬 Uber 量级**：先做前三层 · GNN 看规模决定
- **实时 Feature Store 是刚需** · 开源栈 Feast + Redis / Aerospike 可用
- **图分析可以从批开始**（Neo4j / GraphX）· 实时图是高级需求
...
工业案例规模数字标 `[量级参考]`· 来源：Uber Engineering Blog（Michelangelo / 实时图查询系列）· LinkedIn Engineering Blog。数字为公开披露范围内 · 未独立验证 · 仅作规模量级的参考。
