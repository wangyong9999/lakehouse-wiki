...
!!! tip "一句话理解"
    **向量版分层跳表**。把向量组成**多层图**，上层稀疏下层稠密，查询从顶层入口出发**贪心走下楼**，每层只去最近邻——**log(N) 步**就能走到查询点邻域。在**高召回 + 低延迟**场景是工业界最常用的 ANN 索引之一——**百万-千万级的生产常见默认**；亿级以上常和 IVF-PQ / DiskANN 各有千秋 · 选型按规模和内存预算定。
