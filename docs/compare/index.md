...
- [计算引擎对比](compute-engines.md) —— Trino vs Spark vs Flink vs DuckDB
...
- SPLADE vs BM25（稀疏检索）
- Rerank 模型横比（bge-reranker / Cohere Rerank / Jina Reranker）
- 流处理引擎横比（Flink / Spark Streaming / Kafka Streams / RisingWave）
- 调度系统横比（Airflow / Dagster / Prefect / DolphinScheduler）
