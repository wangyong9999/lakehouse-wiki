...
!!! tip "一句话理解"
    把有限的 GPU 资源**公平、高效、可抢占**地分给训练、推理、探索三类负载。LLM 时代 GPU 成本是 AI 基础设施的最大头（具体占比依团队构成差异大 · 不要直接套用某"50%"数字），调度效率直接 = 成本效率。
...
- **监控 GPU 利用率**：经验目标 > 60% `[来源未验证 · 示意性经验值]` · 低于此值通常说明调度 / 代码 / 数据加载有瓶颈
