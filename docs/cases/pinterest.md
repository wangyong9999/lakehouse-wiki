...
applies_to: Iceberg · PinSage (GNN) · GraphJet · Homefeed · Ads Ranking · PinLater · Dr.Elephant · Querybook · 2020-2026 公开资料
...
!!! info "本页性质 · reference · 非机制 canonical"
    基于 Pinterest Engineering Blog · 论文（PinSage SIGKDD 2018 · GraphJet VLDB 2017）· 公开演讲整理。机制深挖见 [retrieval/multimodal-embedding](../retrieval/multimodal-embedding.md) · [scenarios/recommender-systems](../scenarios/recommender-systems.md)。

!!! abstract "TL;DR"
    - **身份**：**多模推荐系统工业代表** · "**图(Pin) + 文 + 用户行为**"融合推荐最成熟案例
    - **核心技术贡献**：**PinSage**（SIGKDD 2018 · 大规模 GNN 推荐）· **GraphJet**（VLDB 2017 · 早期实时 random walk 推荐）
    ...
    - **最值得资深工程师看的**：§8 深度技术取舍（GraphJet 2014 random walk vs PinSage 2018 GNN 的技术代际 · 为什么 Pinterest 保留早期系统）· §5 多模 embedding 实操

## 1. 为什么这个案例值得学

Pinterest 的独特性：
- **纯多模推荐业务**（和 Netflix / Uber 的数据平台业务不同 · Pinterest **本质是推荐公司**）
- **公开论文质量高**：PinSage（2018）是最被引用的工业 GNN 推荐论文 · GraphJet（2017）是实时图推荐的早期代表
- **技术代际演进公开**：从 GraphJet（2014 random walk）→ PinSage（2018 GNN）→ 后续 Transformer-based 是推荐系统 10 年演进的**活化石**
...
- 2014 **GraphJet** · 实时图 random walk 推荐
- 2018 **PinSage** · 大规模 GNN 推荐（SIGKDD paper）
...
flowchart BT
  ...
  subgraph "索引 + 召回"
    ...
    realtime[实时召回<br/>GraphJet-like]
  end
...
### 5.2 GraphJet · 实时 random walk 推荐（VLDB 2017）

**Pinterest 更早（2014+）的实时推荐系统**：
- **in-memory 图结构**（Pin × User × Board）
- **实时 random walk**（每次请求 1-10ms 内生成推荐）
- **非 GNN** · 纯算法 + 工程优化

**论文**：[*GraphJet: Real-Time Content Recommendations at Twitter*](https://www.vldb.org/pvldb/vol9/p1281-sharma.pdf)（原本 Twitter 发布 · Pinterest 也有实践）

**和 PinSage 的区别**：
- GraphJet · 实时 + 轻量 + 无模型训练
- PinSage · 离线 + 深度 + 需要大量训练

**Pinterest 保留两者**：GraphJet 处理"**实时新鲜度**"需求 · PinSage 处理"**深度相关性**"需求。
...
| 2024+ | PinSage 后继版本（未公开细节）| GNN 推荐持续演进 |
| 2025+ | 多模 RAG 在搜索中的应用（推测）| 搜索 + 生成融合 |
...
### 8.3 取舍 · 保留 GraphJet（2014）的理由

2018 年 PinSage 出现后 · **Pinterest 并没有淘汰 GraphJet**。为什么？
...
GraphJet（2014）和 PinSage（2018）并存是**工业智慧**：新技术不一定淘汰旧技术 · 而是占据新场景。**不要被"新技术就该替换"绑架**。
...
- **[*GraphJet: Real-Time Content Recommendations*](https://www.vldb.org/pvldb/vol9/p1281-sharma.pdf)**（VLDB 2017）
